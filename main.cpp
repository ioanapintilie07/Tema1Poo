...
int caut(const int v[], int n, int poz,
         int x) { //returneaza pozitia primei aparitii a elementului x in vectorul v dupa pozitia poz
    int p = -1, i;
    for (i = poz + 1; i <= n; ++i)
        if (v[i] == x) {
            p = i;
